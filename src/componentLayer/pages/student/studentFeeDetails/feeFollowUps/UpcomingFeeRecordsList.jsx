...
// upcomingFeeRecords
export const UpcomingFeeRecordsListLoader = async ({ request, params }) => {
  const url = new URL(request.url);
  const queryParams = url.search || "?page=1&pageSize=10&search=";
  try {
    const [
      UpcomingFeeRecordsData,
      BranchesData,
      coursesData,
      counsellorsData,
      leadSourceData,
    ] = await Promise.all([
      ERPApi.get(`/fee/upcomingduefeerecords${queryParams}`),
    ...
    const UpcomingFeeRecordsList = UpcomingFeeRecordsData.data || [];
...
const UpcomingFeeRecordsList = () => {
  const data = useLoaderData();
  const submit = useSubmit();
  const fetcher = useFetcher();

  const { leadSourceList, coursesList, BranchsList, counsellorsList, UpcomingFeeRecordsList, } = data;
  ...
  const initialState = [
    ...
    { label: "TO Date", type: "date", inputname: "toDate", value: "" ,min :new Date(new Date().setDate(new Date().getDate() + 1)).toISOString().split('T')[0]},
    {
      label: "Course",
      type: "select",
      inputname: "course",
            ...
            <div className="card-body">
              <div className="table-responsive table-container table-scroll table-card  border-0">
                ...
                <table className="table table-centered align-middle  table-nowrap equal-cell-table table-hover">
                  ...
                  <tbody>
                    ...
                    {
                      UpcomingFeeRecordsList?.upcomingFeeRecords && UpcomingFeeRecordsList?.upcomingFeeRecords?.length > 0 ?
                      UpcomingFeeRecordsList?.upcomingFeeRecords?.map((item, index) => {
                        return (
                          <tr key={index + 1}>
                            <td className="fs-13 black_300 fw-500 lh-xs bg_light ">
                              {(UpcomingFeeRecordsList?.currentPage - 1) * UpcomingFeeRecordsList.pageSize + index + 1}
                            </td>
              ...
              <div className=" mt-3 align-items-center d-flex justify-content-between row text-center text-sm-start">
                <div className="col-sm">
                  <PaginationInfo
                    data={{
                      length: UpcomingFeeRecordsList?.upcomingFeeRecords?.length,
                      start: UpcomingFeeRecordsList?.startStudent,
                      end: UpcomingFeeRecordsList?.endStudent,
                      total: UpcomingFeeRecordsList?.searchResultStudents,
                    }}
                ...
                <div className="col-sm-auto mt-3 mt-sm-0 d-flex">
                  ...
                  <div className="">
                    <Pagination
                      currentPage={UpcomingFeeRecordsList?.currentPage}
                      totalPages={UpcomingFeeRecordsList?.totalPages}
                      loading={navigation?.state === "loading"}
...
export default UpcomingFeeRecordsList