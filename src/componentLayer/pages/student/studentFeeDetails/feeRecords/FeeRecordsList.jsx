...
export const FeeRecordsListLoader = async ({ request, params }) => {
    ...
    try {
        const [
            ...
            ERPApi.get(
                `/student/list_students${queryParams ? queryParams : `?page=1&pageSize=10&search=`
                }`
            ),
        ...
        const FeeRecordsList = FeeRecordsData.data || [];
...
const FeeRecordsList = () => {

    const data = useLoaderData();
    const submit = useSubmit();
    const fetcher = useFetcher();

    const { leadSourceList, coursesList, BranchsList, counsellorsList, FeeRecordsList, } = data;
                        ...
                        <div className="card-body">
                            <div className="table-responsive table-container table-scroll table-card  border-0">
                                ...
                                <table className="table table-centered align-middle  table-nowrap equal-cell-table table-hover">
                                    ...
                                    <tbody className="">
                                        ...
                                        {
                                            FeeRecordsList?.students && FeeRecordsList?.students?.length > 0 ?

                                                FeeRecordsList?.students?.map((item, index) => {
                                                    ...
                                                    return (
                                                        ...
                                                        <tr key={index + 1}>
                                                            <td className="fs-13 black_300 fw-500 lh-xs bg_light ">
                                                                {(FeeRecordsList?.currentPage - 1) *
                                                                    FeeRecordsList?.pageSize +
                                                                    index +
                                                                    1}
                                                            </td>
                                                            <td
                            ...
                            <div className=" mt-3 align-items-center d-flex justify-content-between row text-center text-sm-start">
                                <div className="col-sm">
                                    <PaginationInfo
                                        data={{
                                            length: FeeRecordsList?.students?.length,
                                            start: FeeRecordsList?.startStudent,
                                            end: FeeRecordsList?.endStudent,
                                            total: FeeRecordsList?.searchResultStudents,
                                        }}
                                ...
                                <div className="col-sm-auto mt-3 mt-sm-0 d-flex">
                                    ...
                                    <div className="">
                                        <Pagination
                                            currentPage={FeeRecordsList?.currentPage}
                                            totalPages={FeeRecordsList?.totalPages}
                                            loading={navigation?.state === "loading"}
