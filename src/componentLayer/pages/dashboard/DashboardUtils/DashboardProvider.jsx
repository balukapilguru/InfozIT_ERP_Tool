...
const DashboardProvider = () => {
    ...
    const getLiveUsers = async () => {
        DispatchDashboard(Actions.setLoading("SET_LIVE_USER_COUNT"))

        try {
            const { data, status } = await api.getLiveUsers();

            
            if (status === 200) {
                DispatchDashboard(Actions.setLiveUsers(data, "SET_LIVE_USER_COUNT"))
            }
        }
        catch (error) {
            console.error(error)
